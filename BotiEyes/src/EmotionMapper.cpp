...
namespace
{
    inline int16_t clampInt(int16_t value, int16_t low, int16_t high)
    {
        if (value < low)
            return low;
        if (value > high)
...
namespace BotiEyes
{
    ...
    void EmotionMapper::mapEmotionToExpressionWithAsymmetry(float valence, float arousal, float asymmetry, ExpressionParameters *params)
    {
        ...
        params->yOffset = (int8_t)clampInt(offset, -5, 8);
        ...
        params->spacingAdjust = (int8_t)clampInt(spacing, -3, 1);
