...
- **FR-007a**: Library MUST provide emotion helper methods for common emotions: happy(), sad(), angry(), calm(), excited(), tired(), surprised(), anxious(), content(), curious(), thinking(), confused() with optional intensity parameter (0.0-1.0); helpers map to predefined valence-arousal coordinates while keeping parametric model available for AI integration; thinking() and confused() added per HRI expert review for conversational AI transparency
