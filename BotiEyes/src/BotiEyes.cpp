...
namespace BotiEyes
{
    ...
    // Constructor
    BotiEyes::BotiEyes()
        : displayPtr(nullptr), flushableDisplay(nullptr), emotionState(nullptr), positionState(nullptr), animationState(nullptr), idleBehaviorEnabled(false), lastIdleTrigger(0), baselineValence(0.0f), baselineArousal(0.5f), initialized(false)
    {
    }
    ...
    {
        if (initialized)
            return INVALID_INPUT; // Already initialized
    ...
    void BotiEyes::setDisplay(Adafruit_GFX *display)
    {
        ...
        flushableDisplay = nullptr;
    }

    void BotiEyes::setDisplay(Adafruit_GFX *display, DisplayFlushable *flushable)
    {
        displayPtr = (void *)display;
        flushableDisplay = flushable;
    }
    ...
    void BotiEyes::updateIdleBehavior()
    {
        ...
        if ((int32_t)(now - nextBlinkAt) >= 0)
        {
            uint32_t d = ((uint32_t)platform::randomRange(100, 301) +
                          (uint32_t)platform::randomRange(100, 301)) /
                         2; // mean ~200
            blink((uint16_t)d);
            lastIdleTrigger = now;
        ...
        if ((int32_t)(now - nextMorphAt) >= 0)
        {
            ...
            int16_t vJit = (int16_t)(((int32_t)platform::randomRange(0, 101) +
                                      (int32_t)platform::randomRange(0, 101)) /
                                         2 -
                                     50); // -50..+50
    ...
    {
        if (!displayPtr || !emotionState || !positionState)
            return;
        ...
        if (animationState && animationState->active)
        {
            ...
            {
            case ANIM_BLINK:
                leftOpen = rightOpen = openness;
                break;
            case ANIM_WINK_LEFT:
                leftOpen = openness;
                break;
            case ANIM_WINK_RIGHT:
                rightOpen = openness;
                break;
            default:
                break;
        ...
        if (flushableDisplay != nullptr)
        {
            flushableDisplay->flush();
        }
    }
